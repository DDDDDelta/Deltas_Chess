...
void Board::set_up() {
    this->get_piece({ 1, 1 }) = constant::WhiteRook;
    this->get_piece({ 2, 1 }) = constant::WhiteKnight;
    this->get_piece({ 3, 1 }) = constant::WhiteBishop;
    this->get_piece({ 4, 1 }) = constant::WhiteQueen;
    this->get_piece({ 5, 1 }) = constant::WhiteKing;
    this->get_piece({ 6, 1 }) = constant::WhiteBishop;
    this->get_piece({ 7, 1 }) = constant::WhiteKnight;
    this->get_piece({ 8, 1 }) = constant::WhiteRook;
    this->get_piece({ 1, 8 }) = constant::BlackRook;
    this->get_piece({ 2, 8 }) = constant::BlackKnight;
    this->get_piece({ 3, 8 }) = constant::BlackBishop;
    this->get_piece({ 4, 8 }) = constant::BlackQueen;
    this->get_piece({ 5, 8 }) = constant::BlackKing;
    this->get_piece({ 6, 8 }) = constant::BlackBishop;
    this->get_piece({ 7, 8 }) = constant::BlackKnight;
    this->get_piece({ 8, 8 }) = constant::BlackRook;
    ...
    auto pred_not_attacked = [this, &opponent_pieces, curr](BC coor) {
        return stdrng::none_of(opponent_pieces, [this, coor, curr](const PieceWithMove& pwm) {
            ...
            if (curr.color == E_Color::Black) {
                ...
                attacked_by_pawn =
                        !top_left.on_board() ? false : this->get_piece(top_left) == constant::WhitePawn ||
                                                       !top_right.on_board() ? false : this->get_piece(top_left) == constant::WhitePawn;
                // never call get_piece when !on_board()
            } else {
                BC bottom_left = coor + Vec2(-1, -1);
                BC bottom_right = coor + Vec2(1, -1);
                attacked_by_pawn =
                        !bottom_left.on_board() ? false : this->get_piece(bottom_left) == constant::BlackPawn ||
                                                          !bottom_right.on_board() ? false : this->get_piece(bottom_left) == constant::BlackPawn;
                // same as above
            }
